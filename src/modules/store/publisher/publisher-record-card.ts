...
export class PublisherRecordCard implements DataFDF {
  ...
  #addData(fields: Fields) {
    if (this.data) {
      ...
      if (this.data.inmerced) {
        fields["Date immersed"] =
          this.data.inmerced.toLocaleDateString("es-VE");
      }
      ...
      if (
        ...
        this.data.privileges[0] === Privilege.SERVANT
      ) {
      ...
      if (
        this.data.privileges &&
        this.data.privileges[1] === Privilege.PIONEER
      ) {
  ...
  #addAnualRecords(fields: Fields) {
    if (this.records) {
      for (const [i, a] of this.records.entries()) {
  ...
  #addMounthRecord(
    ...
    record.places &&
      this.#addRecordField(index, fields, "Place", month, record.places);
    record.videos &&
      this.#addRecordField(index, fields, "Video", month, record.videos);
    record.hours &&
      this.#addRecordField(index, fields, "Hours", month, record.hours);
    record.returnVisits &&
      this.#addRecordField(index, fields, "RV", month, record.returnVisits);
    record.studies &&
      this.#addRecordField(index, fields, "Studies", month, record.studies);
    record.remarks &&
      this.#addRecordRemarkField(
        index,
        fields,
        "Remarks",
        month,
  ...
  #addRecordRemarkField(
    ...
    const key =
      index === 1 ? `${type}${monthString}` : `${type}${monthString}_2`;
    fields[key] = value;
  }
}
