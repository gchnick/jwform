...
export class TransactionRecord implements DataDraw<TransactionRecordFormatted> {
  ...
  readonly #numberFormat = Intl.NumberFormat(this.#LOCALE, {
    style: "decimal",
  ...
  getFormattedData(): TransactionRecordFormatted {
    return {
      ...
      type: this.#type,
      worldwideWorkDonations: this.#numberFormat.format(
        this.#worldwideWorkDonations,
      ),
      congregationExpenses: this.#numberFormat.format(
        this.#congregationExpenses,
      ),
  ...
  #otherFormattedTransactions(): OtherTransactionsFormatted | undefined {
    if (this.#otherTransactions === undefined) return undefined;
    ...
    if (this.#otherTransactions)
      for (const [i, other] of this.#otherTransactions.entries()) {
  ...
  #total(): string {
    let acc = 0;
    if (this.#otherTransactions)
      for (const t of this.#otherTransactions) {
        if (t) acc += t.amount;
      }
    const total =
      this.#worldwideWorkDonations + this.#congregationExpenses + acc;
