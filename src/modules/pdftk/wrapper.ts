...
async function isBase64AndWriteFile(file: string) {
    ...
    if(!base64Regex.test(file)) throw StringBase64Invalid('String base64 is invalid\n')
...
function execute(command: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
        exec(command, (error, stdout) => {
            if (error != null) reject(`Pdftk error: ${error}\n`)
            resolve(stdout)
        })
    })
}
 ...
 */
export async function encryptForm(pdf: string, password: string, userPassword?: string) {
    await goToTemporalDirectory()
    const ORIGIN_FILE = await isBase64AndWriteFile(pdf)
    const GENERATE_FILE = generateRandomNameTo()
